...
2. 写明 connector 查找 Codex CLI 的顺序，和 `SANDRONE_CODEX_APP` 的推荐用法。
...
- 文档可能被理解成必须使用 `/Applications/Codex.app`。已通过解析顺序说明保留 `SANDRONE_CODEX_BIN` 和 PATH 方案。
- `launchctl setenv` 只对当前登录会话有效。README 已明确该限制，并提示跨重启可用用户级 LaunchAgent。

## 验证

- `zsh -ic 'printf "SANDRONE_CODEX_APP=%s\n" "$SANDRONE_CODEX_APP"; command -v codex; codex --version'`
- `launchctl getenv SANDRONE_CODEX_APP`

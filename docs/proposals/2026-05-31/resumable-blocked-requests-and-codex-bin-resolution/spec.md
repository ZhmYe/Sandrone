...
- `sandrone resume --request_id <REQ>` 只打印恢复包路径，没有把 `blocked` request 改回可派发状态。下一次 `tick` 会把它当作 terminal status 跳过。

## 目标

- 默认 agent/reviewer connector 必须能通过配置解析 Codex CLI，不写死用户机器上的 app 路径。
- 默认解析顺序必须是: `SANDRONE_CODEX_BIN`、当前 `PATH`、`SANDRONE_CODEX_APP` bundle 内候选 CLI。
...
- 如果 `SANDRONE_CODEX_BIN` 指向可执行文件或 PATH 命令，使用它。
- 如果当前 `PATH` 能找到 `codex`，使用它。
- 如果设置了 `SANDRONE_CODEX_APP`，只在该 bundle 内检查相对候选位置。
...
- 新 workspace 生成的默认 issue-agent 和 reviewer scripts 包含 `SANDRONE_CODEX_BIN`、`SANDRONE_CODEX_APP` 和 `resolve_codex_bin`。

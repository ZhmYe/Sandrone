...
| G2 | Upgrade generated plans/specs/tasks to the same standard. | G1 | Must | `codex-auto-dev plan` emits detailed sections. |
| G3 | Make `plan` create a planning packet. | G1 | Must | `codex-auto-dev plan --name <change-name> --request_id <id>` creates templates under `docs/changes`. |
...
| G8 | Extract a real installable Codex skill. | G4 | Must | `skills/codex-auto-dev-workflow/SKILL.md` is tracked and copied into generated workspaces. |

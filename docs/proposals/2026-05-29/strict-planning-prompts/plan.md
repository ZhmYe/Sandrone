...
| `skills/sandrone/SKILL.md` | add | Store the canonical installable Codex skill as a tracked file. | No | Generated workspace skill equality test. |
...
- Integration assertion: generated workspace skill exactly matches `skills/sandrone/SKILL.md`.
...
- Breaking change: `sandrone new` now treats trailing arguments as project brief, not base branch.
- Breaking change: public workflow commands are now focused on `new`, `update`, `plan`, `start`, and `finish`.
- Migration: existing users can edit `.sandrone/config.toml` if they need a non-main base branch for new projects.

...
可选自动合并是全局队列流程，不是每个 request 完成后立即触发。`sdr tick --auto-merge` 会先刷新所有 `wait-finish` 候选，写出 `.sandrone/state/scheduler/merge-queue.tsv`、`.sandrone/state/scheduler/merge-plan.json` 和 `obsidian/merge/merge-plan.md`，再调用 `tools/merge-plan.sh` 选择本轮优先合并的一个 request。

`tools/merge-plan.sh` 只决定合并优先级，不审计 PR 代码质量；实现质量已经由 code-review gate 负责。真正合并仍只允许走 `sdr pr-merge --auto-merge`：change-doc gate 必须已通过，merge-plan 决策必须是 `ready_for_merge`，`tools/pr-status.sh` 必须返回 `safe`。`open`、`unsafe`、`unsupported` 都不能触发 merge。
...
默认 `.sandrone/config.toml` 中 `parallel_limit = 1`、`auto_merge = false`。需要并发时用 `sdr tick --parallel-limit 2` 或改配置。需要 tick 自动合并时，必须显式设置 `auto_merge = true`、`SANDRONE_AUTO_MERGE=1` 或单次传 `sdr tick --auto-merge`；每轮最多按 merge-plan 处理一个 `wait-finish` PR。
...
- Reviewer gate 必须由外层 `advance`/`tick` 派发和收敛，不得在子 Codex 中嵌套调用 reviewer。运行态会写成 `decomposition-review-running`、`plan-review-running`、`code-review-running` 或 `integration-review-running`。canonical 运行日志和中间产物在 `agents/<reviewer>/runs/<timestamp-REQ-stage-attempt>/`，`.sandrone/state/jobs` 和 `.sandrone/state/reviews` 仅作兼容兜底。

...
本次变更为框架增加全局 workspace registry 和本地 dashboard。用户可以通过 `sandrone dashboard` 或 `sdr dashboard` 在浏览器中查看所有已登记项目，并按项目、需求和 stage 查看关键文件与 review detail。
...
- Registry 位置默认 `~/.sandrone/workspaces.json`，测试和机器人环境可用 `SANDRONE_HOME` 隔离。
...
- 已完成局部红绿测试:
  ...
  - `cargo test sdr_alias_prints_the_same_cli_help --test cli_flow`

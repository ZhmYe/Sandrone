...
本次只更新 README 文档，补齐 dashboard 页面能力和相关命令说明。现在 README 明确说明 `sdr` 别名、dashboard registry 数据来源、页面为空时的刷新方式、三类项目标签语义、stage 展示和局部滚动行为。
...
- 安装章节说明 `sdr` 可替代 `sandrone`。
- 快速开始说明页面为空时进入 workspace 运行 `sdr list`、`sdr upgrade` 或 `sdr dashboard --json`。

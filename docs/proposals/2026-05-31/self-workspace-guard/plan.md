...
1. 确认根目录污染来源，并清理 `.codex-auto-dev/`、`dev/`、`tools/` 和 `.DS_Store`。
2. 增加集成测试，模拟框架源码 checkout，在其中运行 `codex-auto-dev new`，要求失败且不创建运行态目录。
...
- 当前目录存在 `Cargo.toml`，且 package name 是 `codex-auto-dev-workflow`。
- 当前目录存在 `src/main.rs`。
- 当前目录存在 `templates/`。
- 当前目录存在 `skills/codex-auto-dev-workflow/`。

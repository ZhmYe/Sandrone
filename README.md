...
它会把目标 Git 仓库放进独立 workspace，自动抓取需求，拆分为可追溯的 slice，生成计划，隔离 worktree 实现，通过 reviewer gate 后再进入 PR 交付。每一轮都会留下 Obsidian 文档、CodeGraph 上下文、runtime 日志、review JSON 和状态记录，方便人类和后续 agent 接着工作。
...
适合的使用方式是：让 `tick` 自动扫描和推进需求，让 agent 写计划/代码，让 reviewer 严格拦截风险；人类只在 PR、阻塞恢复、策略调整和交付策略上介入。

![Sandrone Agentic Delivery Loops](assets/readme/sandrone-loop-engineering.png)
...
Sandrone 的核心不是一条超长流水线，而是几个不断收敛的工程循环：

- **需求循环**：`sdr update` 抓取并去重外部需求，父 request 先由 DecompositionAgent 拆成 slice DAG。
- **计划循环**：每个可运行 slice 由 PlanAgent 生成计划，再由 PlanReviewer gate 审查；拒绝则回到同一阶段修正。
- **实现循环**：ImplementationAgent 在独立 worktree 中编码、测试、更新 change doc，通过 format/check 后进入 TestReviewer 和 DesignReviewer。
- **交付循环**：所有 slice 通过后，父 request 进入 `wait-update-pr`；`sdr finish` 负责 commit、push、创建或复用 PR。
- **集成循环**：PR 落后或冲突时走 `sdr pr-refresh`，由 RebaseAgent 修复，再经 IntegrationReviewer 回到 PR 更新状态。
- **合并循环**：自动合并默认关闭；开启后 `tick` 会先生成 merge plan，每轮最多选择一个 `wait-finish` PR，且必须 `pr-status=safe` 才能执行 merge。
...
- 自动流程默认停在 `wait-update-pr` 或 `wait-finish`；只有 `tick --auto-merge`、`.sandrone/config.toml` 或 `SANDRONE_AUTO_MERGE=1` 显式开启，才会先生成全局 merge plan，再在 `pr-status=safe` 时调用合并脚本。
...
| `sdr pr-merge --request_id REQ-0001 --auto-merge` | 可选自动合并执行器；必须先由 PR connector 返回 safe，并记录 scheduler decision。 |
...
<workspace>/
  ...
  agents/                 # 各类 agent/reviewer/merge-planner 的独立运行日志和中间产物
  tools/                  # 可替换 connector 脚本
  .sandrone/              # 机器索引、事件流、锁和兼容指针
  .env                    # workspace 级运行时兜底配置
```

文档图谱规则很简单：`project.md -> 父 request index -> slice index -> 阶段总文档`。大段计划、实现说明和 reviewer 机器 JSON 不复制到 index；重要说明留在 Obsidian，日志、review context、merge queue、runtime JSON 等中间产物放在 `agents/<kind>/runs/**`。
...
- 每种 agent/reviewer 的统一配置文件在 `agents/config/<kind>.json`（当前 kind 包括 decomposition/plan/implementation/rebase-agent 及各 reviewer），可以分别设置 backend、model、reasoning、api_key 和 base_url。
